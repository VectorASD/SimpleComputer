...
int bc_box(int x1, int y1, int w, int h) { // К сожалению автор ТЗ перепутал X и Y, а также перепутал x2, y2 с w, h, что
...
void bc_printKeys() {
    int X = 55, Y = 17;
    text keys[] = {"l   - load          enter  - edit", "s   - save          arrows - move", "r   - run", "t   - step", "i   - reset", "F5  - accumulator", "F6  - instructionCounter", "ESC - leave"};
    for (int i = 0; i < 8; i++) {
        mt_gotoXY(Y + i, X);
        my_printf("%s", keys[i]);
    }
...
void bc_printAccumulator(int accumulator, int current) {
    ...
    mt_gotoXY(5, 80);
    my_printf("%c%04x", accumulator >> 14 & 1 ? '-' : '+', accumulator & 0x3fff);
    if (current)
        mt_clrclr();
}
