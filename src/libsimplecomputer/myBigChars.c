...
void bc_printMemory(int X, int Y, int current) {
    ...
    for (int mem = 0; mem < MEMORY_SIZE; mem++) {
        ...
        printf("%c%04X", value >> 14 & 1 ? '-' : '+', value & 0x3fff);
        if (mem == current) mt_clrclr();
    }
}
void bc_printFlags(int X, int Y) {
...
void bc_printBigNumbers(int X, int Y, int num, Color a, Color b) {
    if (num >= 0 && num < MEMORY_SIZE) sc_memoryGet(num, &num);
