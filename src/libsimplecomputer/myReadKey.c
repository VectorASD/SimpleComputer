...
byte upper_comparer(char *buff, text right) {
  for (int i = 0; ; i++) {
    char c = buff[i], c2 = right[i];
    if (c >= 'a' && c <= 'z') c -= ' ';
    if (c != c2) return 0;
    if (c == 0) return 1; // по сути аналогично c == 0 && c2 == 0
  }
}
...
rk_readkey (Keys *key)
{
  ...
  char buff[7] = "\0\0\0\0\0\0"; // 7 байт - всегда нулевой терминатор - предохранитель
...
rk_mytermsave()
{
  if (tcgetattr(fileno(stdin), &rk_save)) return 1;
  return 0;
}
...
// sigint: 1 - можно использовать ctrl+... alt+..., включая (intr), (quit), 0 - на прямую сыпятся ctrl и alt, как отдельные символы

int
rk_mytermregime (int regime, int vtime, int vmin, int echo, int sigint)
{
  if (regime != !!regime) return 1;
  if (vtime < 0 || vmin < 0 || (echo != !!echo) || (sigint != !!sigint)) return 2;
  Term term;
  if (tcgetattr(fileno(stdin), &term)) return 3;
  
  if (!!(term.c_lflag & ICANON) != regime) term.c_lflag ^= ICANON;
  if (!!(term.c_lflag & ISIG) != sigint) term.c_lflag ^= ISIG;
  if (!!(term.c_lflag & ECHO) != echo) term.c_lflag ^= ECHO;
  term.c_cc[VMIN] = vmin;
...
void rk_test() {
	...
	for (int i = 0; i < 5; i++) {
	  Keys res;
	  rk_readkey(&res);
	  if (res != K_OTHER) printf("Key_num: %u\n", res);
	  else printf("Other_key\n");
	}
