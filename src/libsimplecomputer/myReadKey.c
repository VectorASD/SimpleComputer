...
void apply_vvod(int num) {
    if (mem_pos >= 0 && mem_pos <= MEMORY_SIZE)
        sc_memorySet(mem_pos, num);
    else if (mem_pos == -1)
        accumulator = num;
    else if (mem_pos == -2) {
        // хм... чтобы форматирование не глючило
        if (num >= MEMORY_SIZE)
            instruction = MEMORY_SIZE - 1;
        else
            instruction = num < 0 ? 0 : num;
    }
...
void rk_test() // Только для lab04. Перенесено в main5 для дальнейшего
               // редактирования!
{
    // rk_print();
    if (rk_mytermsave()) {
